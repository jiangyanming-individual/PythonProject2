...
MODEL_PATH = "xgboost_local_model.pkl"  # .pkl模型文件路径
TEST_CSV_PATH = "test_data.csv"  # CSV测试集路径
PREDICTION_SAVE_PATH = "prediction_result.csv"  # 预测结果保存路径
LABEL_COLUMN = "出料流量"

def predict_with_xgboost_pkl():
    try:
        # 1. 加载 .pkl 原生XGBoost模型
        print(f"正在加载模型：{MODEL_PATH}")
        model = joblib.load(MODEL_PATH)
        print("模型加载成功！")

        # 2. 读取 CSV 测试集
        print(f"\正在读取测试集：{TEST_CSV_PATH}")
        test_df = pd.read_csv(TEST_CSV_PATH)
        print(f"测试集读取成功，数据形状：{test_df.shape}")
        print(f"测试集前5行预览：\n{test_df.head()}")

        # 3. 数据预处理：分离特征和标签（若有标签列）
